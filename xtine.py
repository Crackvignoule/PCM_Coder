...
sample_rate = 8000  # Standard audio sample rate
...
packets = [
    quantized_signal_8bit[i : i + packet_size]
    for i in range(0, len(quantized_signal_8bit), packet_size)
]
...
def simulate_packet_loss(packets, loss_probability):
    ...
    for packet in packets:
        if np.random.rand() > loss_probability:
            ...
            lost_packets.append(
                np.zeros_like(packet)
            )  # Simulate lost packet with zeros
...
lost_packets_count = sum(
    1 for packet in packets if np.array_equal(packet, np.zeros_like(packet))
)
